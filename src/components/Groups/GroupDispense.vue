...
let curGroupPackHeader = ref(new GroupPackHeader({ id: uuidv4() }));
...
const fechtPatientVisitDetails = (pack) => {
  patientVisitDetailsService
    .apiFetchById(pack.patientVisitDetails.id)
    .then((resp) => {
      pack.patientVisitDetails = PatientVisitDetails.find(
        pack.patientVisitDetails.id
      );
    });
};
