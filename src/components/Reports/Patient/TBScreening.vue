...
const initReportProcessing = async (params) => {
  ...
  if (isOnline.value) {
    ...
    try {
      await PatientsWithScreeningMobileService.getDataLocalDbForTBScreening(
        params
      );
      progress.value = 100;
      params.progress = 100;
      ...
      console.error(
        'Unable to generate the offline TB screening report',
        error
      );
      progress.value = 0;
      params.progress = 0;
...
const generateReport = async (id, fileType) => {
  if (isOnline.value) {
    ...
    if (data.length === 0) {
      ...
      if (fileType === 'PDF') {
        TBScreeningReportTs.downloadPDF(
          patientAux.province,
