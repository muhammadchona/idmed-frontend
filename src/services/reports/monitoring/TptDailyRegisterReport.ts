...
const title = 'LIVRO DE REGISTO DIÁRIO DE TPTs';
const reportName = 'TptDailyListReport';
...
export default {
  async downloadPDF(id, fileType, params) {
    ...
    const headerReport = [
      [
        {
          content: 'LIVRO DE REGISTO DIÁRIO DE TPTs',
  ...
  async getDataLocalReport(reportId) {
    const reportData =
      await TptDailyRegisterMobileService.localDbGetAllByReportId(reportId);
