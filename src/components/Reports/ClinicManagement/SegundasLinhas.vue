...
import  segundasLinhasTS from 'src/services/reports/ClinicManagement/SegundasLinhas';
...
const generateReport = (id, fileType) => {
  if (fileType === 'PDF') {
    segundasLinhasTS.downloadPDF(id).then((resp) => {
      if (resp === 204)
        alertError('Não existem Dados para o período selecionado');
      downloadingPdf.value = false;
    });
  } else {
    segundasLinhasTS.downloadExcel(id).then((resp) => {
      if (resp === 204)
        alertError('Não existem Dados para o período selecionado');
