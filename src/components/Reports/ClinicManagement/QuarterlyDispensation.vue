<template>
  <div ref="filterDPatientHistorySection">
    ...
    v-if="resultFromLocalStorage"
      ...
      bgColor="bg-orange-5">Serviço {{serviceAux !== null ? serviceAux.code : ''}}: Pacientes em Dispensa Trimestral
    </ListHeader>
    <ListHeader
    v-else
      :addVisible="false"
      ...
      :closeVisible="true"
      @closeSection="closeSection(params)"
      bgColor="bg-orange-5">Serviço {{selectedService !== null ? selectedService.code : ''}}: Pacientes em Dispensa Trimestral
    </ListHeader>
  ...
  <script setup>
    ...
    import quarterlyDispensationTS from 'src/services/reports/ClinicManagement/QuarterlyDispensation.ts'
    ...
    const generateReport = (id, fileType) => {
      //  UID da tab corrente
      if (isOnline.value) {
        Report.printReport('activePatientReport', id, fileType).then(
          (resp) => {
            if (!resp.data[0]) {
              ...
              if (fileType === 'PDF') {
                ...
                quarterlyDispensationTS.downloadPDF(
                  firstReg.province,
                ...
                quarterlyDispensationTS.downloadExcel(
                  firstReg.province,
                  moment(new Date(firstReg.startDate)).format('DD-MM-YYYY'),
                  moment(new Date(firstReg.endDate)).format('DD-MM-YYYY'),
                  resp.data,
