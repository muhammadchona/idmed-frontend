...
export default {
  ...
  methods: {
      ...
      if (this.mobile) {
        ...
        ClinicalService.localDbGetAll().then((clinicalServices) => {
          clinicalServices.forEach((clinicalService) => {
            clinicalService.identifier_type_id =
              clinicalService.identifier_type.id;
    ...
    search() {
      const groups = Group.query().with('groupType').with('service').get();
      this.searchResults = groups.filter((group) => {
        return (
          this.stringContains(group.code, this.curGroup.code) ||
          this.stringContains(group.name, this.curGroup.name)
        );
      });
    },
    openGroup(group) {
      ...
      patientList.forEach((patient) => {
        patient.identifiers.forEach((identifier) => {
          ...
          identifier.episodes.forEach((episode) => {
            PatientVisitDetails.apiGetAllByEpisodeId(episode.id, 0, 300).then(
              (resp) => {
