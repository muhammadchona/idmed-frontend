...
export default {
  ...
  // Mobile
   ...
   localDbGetAll () {
   return nSQL('drugFiles').query('select').exec().then(result => {
   ...
   calculateBalance (records: any) {
    ...
    records.forEach(item => {
      const current = Number(item.incomes) - Number(item.outcomes) + Number(item.posetiveAdjustment) - Number(item.negativeAdjustment) - Number(item.loses)
  ...
  async getDrugFileSummary (drug: any) {
          ...
          const inventoryAdjustments = await stockMethod.getInventoryAdjustmentsDrugFile(drug)
  ...
  async getDrugFileSummaryBatch (stockId: any) {
      ...
      const destructions = await stockMethod.getDestructionsDrugFileBatch(stockId)
      destructions.forEach(item => {
        list.push(item)
      })

      const inventoryAdjustments = await stockMethod.getInventoryAdjustmentsDrugFileBatch(stockId)
      inventoryAdjustments.forEach(item => {
        list.push(item)
      })
      list = list.sort((a, b) => {
