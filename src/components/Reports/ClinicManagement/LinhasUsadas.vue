<template>
  <div ref="filterLinhasUsadasSection">
    <ListHeader
      ...
      bgColor="bg-orange-5"
    >Serviço {{ serviceAux !== null ? serviceAux.code : '' }}: Linhas Terapeuticas Usadas
    </ListHeader>
    <ListHeader
      v-else
      :addVisible="false"
      ...
      @closeSection="closeSection(params)"
      bgColor="bg-orange-5"
    >Serviço {{ selectedService !== null ? selectedService.code : '' }}: Linhas Terapeuticas Usadas
    </ListHeader>
...
import linhasUsadasReport from 'src/services/reports/ClinicManagement/LinhasUsadas';
...
const name = 'LinhasTerapeuticasUsadas';
const props = defineProps(['selectedService', 'menuSelected', 'id', 'params']);
const totalRecords = ref(0);
const qtyProcessed = ref(0);
const filterLinhasUsadasSection = ref('');
const downloadingPdf = ref(false);
const downloadingXls = ref(false);
const reportType = 'LINHAS_TERAPEUTICAS_USADAS';
...
const closeSection = (params) => {
  filterLinhasUsadasSection.value.remove();
  if (params) {
...
const initReportProcessing = async (params) => {
  ...
  if (isOnline.value) {
    ...
    Report.apiInitLinhasUsadasProcessing(params).then((resp) => {
...
const getProcessingStatus = (params) => {
  Report.getProcessingStatus('linhasUsadasReport', params).then((resp) => {
...
const generateReport = (id, fileType) => {
  if (fileType === 'PDF') {
    linhasUsadasReport.downloadPDF(id).then((resp) => {
      console.log(resp)
      if (resp === 204)
    ...
    linhasUsadasReport.downloadExcel(id).then((resp) => {
      if (resp === 204)
        alertError('Não existem Dados para o período selecionado');
