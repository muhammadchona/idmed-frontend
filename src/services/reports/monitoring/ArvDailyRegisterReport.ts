...
export default {
  async downloadPDF (id, fileType, params) {
   ...
   params.endDateParam = Report.getFormatDDMMYYYY(firstReg.endDate)
    ...
    const headerReport = [
      ...
      [
        {
          content: 'Distrito: ' + (params.district === null ? clinic.district.description : params.district.description),
          halign: 'center',
          valign: 'middle',
          fontStyle: 'bold',
          fontSize: '14'
        ...
        {
          content: 'Província: ' + (params.province === null ? clinic.province.description : params.province.description),
          halign: 'center',
          valign: 'left',
          fontStyle: 'bold',
          fontSize: '14'
 ...
 if (data.row.section === 'body' && data.column.dataKey === 10) {
      console.log(rowsAux)
      const dataRow = rowsAux.data[data.row.index]
 ...
 theme: 'grid',
  ...
  async downloadExcel (id, fileType2, params) {
    ...
    const clinic = clinicService.currClinic()
    const rows = await Report.printReport('arvDailyRegisterReportTemp', id,fileType2) 
 ...
 // merge a range of cells
  ...
  cellProfilaxia.font = {
   ...
   // let x = 14
    ...
    let p = 15
    console.log('(rows.data).length: ', (rows).length)
     // Loop through all table's row
     for (let j = 0; j <= (rows.data).length; j++) {
     // const row = worksheet.getRow(i)
      // Now loop through every row's cell and finally set alignment
       const reportData = (rows.data)[j]
      //  console.log('ReportDataLenght: ', (rows.data).length)
