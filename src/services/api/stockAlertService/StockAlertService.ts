...
export default {
  ...
  async apiGetStockAlertAll(clinicId: string) {
    if (isMobile.value) {
      // The tablet uses the same operational per-lot balance used to validate
      // a dispense. It never rebuilds the current balance from historical
      // movements and it never contacts the backend while working offline.
      const response = await this.localDbGetStockAlertMobile(clinicId);
  ...
  async localDbGetStockAlertMobile(clinicId: string) {
    ...
    for (const drug of drugList) {
      const hasStock = await StockService.hasStockMobile(drug, clinicId);
      if (hasStock) {
        const stockAlert = new StockAlert();
        // stockMoviment is the tablet's operational source of truth. A manual
        // local entrance establishes the balance and later offline operations
        // update it directly.
        const balance =
