...
export default {
  saveReferredPatientsOnNano(
    referredPatientsResult: any,
    reportParams: any,
    clinic: any
  ) {
    ...
    for (const reportData of referredPatientsResult) {
      ...
      referredPatient.name =
        reportData.firstNames +
        ' ' +
        reportData.middleNames +
        ' ' +
        reportData.lastNames;
      referredPatient.age = idadeCalculator(
        new Date(getDDMMYYYFromJSDate(new Date(reportData.dateOfBirth)))
      );
      referredPatient.referrenceDate = reportData.referenceDate;
      referredPatient.nextPickUpDate = reportData.nextPickUpDate;

      const prescriptionAux = prescriptionService.getPrescriptionByID(
        reportData.prescriptionId
      );
      const therapeuticRegAux = therapeuticalRegimenService.getById(
        prescriptionAux?.prescriptionDetails?.[0]?.therapeutic_regimen_id
      );
      const therapeuticLineAux = therapeuticLineService.getById(
        prescriptionAux?.prescriptionDetails?.[0]?.therapeutic_line_id
      );
      const dispenseTypeAux = dispenseTypeService.getById(
        prescriptionAux?.prescriptionDetails?.[0]?.dispense_type_id
      );
      if (therapeuticRegAux)
        referredPatient.therapeuticRegimen = therapeuticRegAux.code;
      if (therapeuticLineAux)
        referredPatient.therapeuticLine = therapeuticLineAux.code;
      if (dispenseTypeAux) referredPatient.dispenseType = dispenseTypeAux.code;

      referredPatient.referralPharmacy = reportData.referralClinic?.clinicName;

      referredPatient.lastPrescriptionDate =
  ...
  localDbAddOrUpdate(targetCopy: any) {
    return nSQL(ReferredPatientsReport.entity)
      .query('upsert', JSON.parse(JSON.stringify(targetCopy)))
      .exec();
  },
