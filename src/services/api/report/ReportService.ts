...
export default {
 ...
 return api().get(`/historicoLevantamentoReport/printReport/${id}/${fileType}`, { responseType: 'json' })
  ...
  getRegisteredPatientByDispenseType(year: any, clinicId: string, serviceCode: string) {
    if (isOnline.value) {
      ...
      const serviceCD = serviceCode
        ...
        .query('select', ['1 AS month', 'clinicalServices.code AS service', 'identifiers.startDate AS startDate', 'patientVisits.visitDate AS visitDate',
        'patientVisits.patient.id AS patientId', 'dispenseTypes.code AS dispenseTypeCode'])
        .join(
          [{
            type: 'inner',
            table: 'identifiers',
          ...
          },
          {
            type: 'inner',
            table: 'prescriptions',
            where: ['prescriptions.id', '=', 'patientVisitDetails.prescription.id']
          },
          {
            type: 'inner',
          ...
          },
          {
            type: 'inner',
            table: 'dispenseTypes',
            where: ['dispenseTypes.id', '=', 'prescriptionsDetails.dispenseType.id']
          }]
        )
  ...
  getPatientsFirstDispenseByAge(year: any, clinicId: string, serviceCode: string) {
    if (isOnline.value) {
      return api().get(
        `/dashBoard/getPatientsFirstDispenseByAge/${year}/${clinicId}/${serviceCode}`
      );
  ...
  getPatientsFirstDispenseByGender(year: any, clinicId: string, serviceCode: string) {
    if (isOnline.value) {
      return api().get(
        `/dashBoard/getPatientsFirstDispenseByGender/${year}/${clinicId}/${serviceCode}`
      );
  ...
  agruparRegistrosPorDispenseTypeCodeMonthGender (registros: any) {
    const agrupadoPorDispenseTypeCode = registros.reduce((agrupado: any, registro: any) => {
  ...
  getDispenseByAge(year: any, clinicId: string, serviceCode: string) {
    if (isOnline.value) {
      ...
      const serviceCD = serviceCode
      return nSQL('patientVisits')
      .query('select', ['identifiers.startDate AS startDate', 'patientVisits.visitDate AS visitDate', 'packs.pickupDate AS pickupDate',
      ...
      'dispenseTypes.code AS dispenseTypeCode', '0 AS faixa',
