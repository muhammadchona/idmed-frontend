...
export default {
  async downloadPDF(id) {
    ...
    let segundasLinhasData = [];
    let matriz = [[]];
    if (isOnline.value) {
      const segundasLinhasReport = await Report.printReportOther('segundasLinhasReport', id);
      if (segundasLinhasReport.status === 204 || !segundasLinhasReport.data.length) return 204;
      segundasLinhasData = segundasLinhasReport.data;
      const agrupado = agruparPorRegime(segundasLinhasReport.data);
    ...
    const headData = [
      ...
      [
        ...
        {
          content: 'Data Fim: ' + Report.getFormatDDMMYYYY(segundasLinhasData[0].endDate),
          styles: { valign: 'middle', halign: 'center', fontStyle: 'bold', textColor: 0 }
        }
      ]
    ];
  ...
  async downloadExcel(id) {
    const clinic = clinicService.currClinic();
    console.log(clinic)
    let segundasLinhasReport = {};
    let segundasLinhasData = [];
    let matriz = [[]]
    if (isOnline.value) {
      segundasLinhasReport = await Report.printReportOther('segundasLinhasReport', id);
      console.log(segundasLinhasReport)
      if (segundasLinhasReport.status === 204 || segundasLinhasReport.data.length === 0) return 204;
      segundasLinhasData = segundasLinhasReport.data;
      const agrupado = agruparPorRegime(segundasLinhasReport.data);
      matriz = transformarEmMatriz(agrupado);
      console.log(matriz)
    ...
    cellStartDateParamValue.value = Report.getFormatDDMMYYYY(segundasLinhasData[0].startDate);
    cellEndDateParamValue.value = Report.getFormatDDMMYYYY(segundasLinhasData[0].endDate);;
