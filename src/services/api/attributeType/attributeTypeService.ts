...
export default {
  ...
  async postWeb(params: string) {
    try {
      const resp = await api().post('attributeType', params);
  ...
  async patchWeb(uuid: string, params: string) {
    try {
      const resp = await api().patch('attributeType/' + uuid, params);
      attributeType.save(resp.data);
  ...
  async deleteWeb(uuid: string) {
    try {
      const resp = await api().delete('attributeType/' + uuid);
      attributeType.destroy(uuid);
  ...
  putMobile(params: string) {
    return nSQL(attributeType.use?.entity)
      .query('upsert', params)
      .exec()
      .then(() => {
