...
export function useStock() {
  ...
  function getEntrancesDrugFile(drug: any) {
    ...
    return nSQL('stocks')
      .query('select', [
        'SUM(stocks.unitsReceived) AS stockEntrances.incomes',
        'stockEntrances.dateReceived',
        'stockEntrances.orderNumber',
      ])
      .where(['drug_id', '=', drug.id])
      .join({
        type: 'inner',
              ...
              for (const pcd of pvdObj.pack.packagedDrugs) {
                if (pcd.drug_id === drug.id) {
                  ...
                  for (const pcdStockObj of pcd.packagedDrugStocks) {
                    drugQuantitySupplied += Number(
                      pcdStockObj.quantitySupplied
                    );
                  }
                  recordFile.id = uuidv4();
    ...
    return nSQL('inventoryStockAdjustments')
      ...
      .then((result) => {
        for (const adjustment of result) {
          const recordFile = {};
          if (
            adjustment['inventoryStockAdjustments.adjustedStock'].id === stockId
          ) {
            recordFile.id = uuidv4();
            recordFile.eventDate =
              adjustment['inventoryStockAdjustments.captureDate'];
            recordFile.moviment = 'Inventário';
            recordFile.orderNumber = '';
            recordFile.incomes = 0;
            recordFile.outcomes = 0;
            recordFile.posetiveAdjustment =
              adjustment['inventoryStockAdjustments.operation'].code ===
              'AJUSTE_POSETIVO'
                ? adjustment['inventoryStockAdjustments.adjustedValue']
                : 0;
            recordFile.negativeAdjustment =
              adjustment['inventoryStockAdjustments.operation'].code ===
              'AJUSTE_NEGATIVO'
  ...
  function getEntrancesDrugFileBatch(stockId: string) {
    ...
    return nSQL('stocks')
      .query('select', [
        'SUM(stocks.unitsReceived) AS incomes',
        'stocks.id',
        'stockEntrances.dateReceived',
        'stockEntrances.orderNumber',
      ])
