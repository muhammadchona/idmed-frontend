<template>
  <div ref="filterGeneralInventorySection">
    <ListHeader
...
import InventoryGeneralTS from 'src/services/reports/stock/InventoryGeneral.ts';
...
const name = 'InventoryGeneral';
const props = defineProps(['selectedService', 'menuSelected', 'id', 'params']);
const totalRecords = ref(0);
const qtyProcessed = ref(0);
const report = 'INVENTARIO_GENERAL';
const progress = ref(0.0);
const filterGeneralInventorySection = ref('');
...
const closeSection = (params) => {
  filterGeneralInventorySection.value.remove();
  if (params) {
...
const printReportInv = (id, reportId, fileType) => {
  ...
  Report.apiPrintInventoryReport(id, reportId).then((resp) => {
    if (!resp.data[0]) {
      ...
      if (fileType === 'PDF') {
        InventoryGeneralTS.downloadPDF(
          patientAux.province,
        ...
        InventoryGeneralTS.downloadExcel(
          patientAux.province,
          patientAux.startDate,
          patientAux.endDate,
          resp.data
