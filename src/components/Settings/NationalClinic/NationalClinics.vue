    ...
    <div class="">
      ...
      <div class="q-mb-md text-weight-bold text-subtitle1">
        <q-bar style="background-color: #9e9e9e2e">
          <div class="cursor-pointer non-selectable">Unidade Sanitaria</div>
        </q-bar>
      </div>
      <q-table
        style="height: 700px"
...
export default {
  ...
  computed: {
    getNationalClinicos() {
      return NationalClinic.query().with('province.districts').with('facilityType').get();
    },
  },
  methods: {
    ...
    promptToConfirm(nationalClinic) {
      ...
      this.$q
        .dialog({
          title: 'Confirm',
          message: nationalClinic.active
            ? 'Deseja Inactivar a Unidade Sanitária?'
            : 'Deseja Activar a Unidade Sanitária?',
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
          ...
          NationalClinic.apiSave(nationalClinic)
            .then((resp) => {
