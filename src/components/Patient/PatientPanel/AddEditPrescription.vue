  ...
  <q-card style="width: 1350px; max-width: 110vw">
    <q-card-section style="max-height: 50vh" class="q-pa-none bg-green-2">
      <div class="row items-center text-subtitle1 q-pa-md">
        ...
        <div class="text-grey-10 q-ml-sm" v-if="patient.age() <= 14">
          <span class="text-bold text-h6">
            | <q-icon name="child_care" />
          </span>
          {{ patient.age() }} Ano(s) de Idade
        </div>
        <div class="text-grey-10 q-ml-sm" v-else>
          <span class="text-bold text-h6">|</span> {{ patient.age() }} Anos de
          Idade
        </div>
      </div>
      <q-separator />
    ...
    <q-card-section>
      ...
      >
        <span>
          <span
            v-for="clinicalService in clinicalServices"
          ...
          >
            <ListHeader
              :editVisible="
                !(
                  ...
                  clinicalService.id === selectedClinicalService.id
                )
              "
              :doneVisible="
                selectedClinicalService !== null &&
            ...
            >
              ...
              {{
                this.patient.identifiers.find(
                  (identifier) =>
                    identifier.service.code === clinicalService.code
                ).identifierType.code
              }}:
              {{
                this.patient.identifiers.find(
                ...
                this.patient.identifiers.find(
                  ...
                  : this.patient.identifiers.find(
                      (identifier) => identifier.prefered === true
                    ).value
              }}
            </ListHeader>
            <div
              class="box-border q-pa-sm"
              v-if="
                selectedClinicalService !== null &&
                clinicalService.id === selectedClinicalService.id
              "
                ...
                <div class="q-ma-md">
                    ...
                    <div class="col">
                      <div class="row">
                        <q-checkbox
                          ...
                          :disable="
                            mustBeSpetial || isNewPackStep || isEditPackStep
                          "
                        ...
                        <q-select
                          ...
                          v-if="
                            spetialPrescription ||
                            curPrescriptionDetails.spetialPrescriptionMotive !==
                              null
                          "
                          v-model="
                            curPrescriptionDetails.spetialPrescriptionMotive
                          ...
                          :rules="[
                            (val) =>
                              !!val ||
                              'Por favor indicar o motivo da prescrição especial',
                          ]"
                          option-value="id"
                  ...
                  <div class="row">
                    <q-select
                      ...
                      :rules="[
                        (val) =>
                          !!val || 'Por favor indicar o regime terapêutico',
                      ]"
                    ...
                    <q-select
                      ...
                      :rules="[
                        (val) =>
                          !!val || 'Por favor indicar a linha terapêutica',
                      ]"
                  ...
                  <div class="row">
                    ...
                    <q-select
                      ...
                      :rules="[
                        (val) =>
                          !!val || 'Por favor indicar o tipo de dispensa',
                      ]"
          ...
          <div
            class="row q-mt-xs"
            v-if="this.member === null || this.member === undefined"
          >
            <q-banner
              dense
              inline-actions
...
const therapeuticRegimenRef = ref('');
const mds = ref(ref('US_'));
const identifiers = ref([]);
const patientVisit = ref(new PatientVisit());
const curPatientVisitDetails = ref([]);
const curPatientVisitDetail = ref(null);
const clinicalServices = ref([]);
const selectedClinicalService = ref(null);
const reasonsForUpdate = ref(['FT', 'Alergia']);
const patientStatus = ref(['Inicio', 'Manutenção']);
const patientTypes = ref(['Sim', 'Não']);
const showServiceDrugsManagement = ref(false);
const prescriptionDate = ref(moment().format('DD-MM-YYYY'));
...
// Injection
const selectedVisitDetails = inject('selectedVisitDetails');
...
const getLastPrescriptionData = () => {
  ...
  if (identifier.lastVisitPrescription() !== null) {
    const lastVisitPrescription =
      identifier.lastVisitPrescription().prescription;
    this.curPrescriptionDetails.therapeuticRegimen =
      lastVisitPrescription.prescriptionDetails[0].therapeuticRegimen;
    this.curPrescriptionDetails.therapeuticLine =
      lastVisitPrescription.prescriptionDetails[0].therapeuticLine;
    this.curPrescriptionDetails.duration =
      lastVisitPrescription.prescriptionDetails[0].duration;
    this.curPrescription.duration = lastVisitPrescription.duration;
    this.curPrescription.doctor = Doctor.query()
      .whereId(lastVisitPrescription.doctor_id)
      .first();
    this.curPrescription.patientStatus =
      lastVisitPrescription.patientStatus === 'Inicio'
        ? 'Manutenção'
        : 'Inicio';
    this.curPrescriptionDetails.dispenseType =
      lastVisitPrescription.prescriptionDetails[0].dispenseType;
...
const validateDate = () => {
  ...
  if (identifier.lastVisitPrescription() !== null) {
    ...
    const prescriptionDate = moment
      .utc(
        date.addToDate(
          this.curPatientVisitDetail.prescription.prescriptionDate,
          { days: 4 }
        )
      )
      .local()
    ...
    if (momentNextPickUpDate.isAfter(momentPrescriptionDate)) {
      this.displayAlert(
        'confirmation',
        'O paciente ainda possui medicamentos em casa para o serviço de ' +
          identifier.service.description +
          ', deseja  continuar com a criação da nova prescrição?'
      );
    }
  }
};
const getIdentifierWithAll = () => {
  return PatientServiceIdentifier.query()
    .with([
      'episodes.patientVisitDetails.pack',
      'episodes.patientVisitDetails.prescription.duration',
      'episodes.patientVisitDetails.prescription.prescriptionDetails.*',
    ])
    .with('service')
...
const initPatientVisitDetailsForDispense = () => {
  ...
  this.curPatientVisitDetail = PatientVisitDetails.query()
    ...
    .first();
  this.curPatientVisitDetail.prescription.prescriptionDetails =
    PrescriptionDetail.query()
      .withAll()
      .where(
        'id',
        this.curPatientVisitDetail.prescription.prescriptionDetails[0].id
      )
      .get();
  this.prescriptionDate = this.getDDMMYYYFromJSDate(
    this.curPatientVisitDetail.prescription.prescriptionDate
  );
  this.selectedClinicalService = ClinicalService.query()
    ...
    .where(
      'id',
      this.curPatientVisitDetail.episode.patientServiceIdentifier.service.id
    )
    .first();
  this.curPatientVisitDetail.prescription.prescribedDrugs =
    PrescribedDrug.query()
      .with('drug.form')
      .where('prescription_id', this.curPatientVisitDetail.prescription.id)
      .get();
   prescribedDrugs.value = this.curPatientVisitDetail.prescription.prescribedDrugs;
  let packagedDrugs = null;
  if (
    this.curPatientVisitDetail.prescription.prescriptionDetails[0]
  ...
  if (this.isEditPackStep || this.isFirstPack) {
    ...
    prescribedDrugs.value.forEach((prescribedDrug) => {
      packagedDrugs.forEach((packagedDrug) => {
        if (
          this.isNewPackStep &&
          prescribedDrug.drug.id === packagedDrug.drug.id &&
          packagedDrug.toContinue
        ) {
  ...
  if (this.selectedVisitDetails !== null) {
    this.curPatientVisitDetail.createPackLater =
      this.selectedVisitDetails.createPackLater;
  }
  SessionStorage.set(
    this.curPatientVisitDetail.episode.patientServiceIdentifier.service.code,
    this.curPatientVisitDetail
  );
  this.clinicalServices.push(
    this.curPatientVisitDetail.episode.patientServiceIdentifier.service
  );
...
const getPatientActiveClinicalServices = () => {
  if (!this.isNewPackStep && !this.isEditPackStep) {
    const identifiers = this.patient.identifiers.filter((identifier) => {
      if (this.service !== null && this.service !== undefined)
        return identifier.service.code === this.service.code;
    ...
    identifiers.forEach((identifier) => {
      ...
      if (
        episode !== null &&
        (!episode.closed() || episode.isDCReferenceEpisode())
      ) {
        this.clinicalServices.push(
          ClinicalService.query()
            .with('attributes.*')
            .where('id', identifier.service.id)
            .first()
        );
...
const initPatientVisitDetails = (episode) => {
  const pack = new Pack({
    clinic: this.currClinic,
  ...
  curPatientVisitDetail.createPackLater =
    this.selectedVisitDetails.createPackLater;
...
const initEdition = (clinicalService) => {
  this.inFormEdition = true;
...
const isSelectedService = (clinicalService) => {
...
const alterPatientStatus = () => {
  if (
    String(this.curPrescriptionDetails.dispenseType.description).includes(
      'Mensal'
    )
  ) {
...
const saveCurPatientVisitDetails = () => {
  if (this.curPatientVisitDetail.episode.startStopReason === null)
    this.curPatientVisitDetail.episode.startStopReason = StartStopReason.find(
      this.curPatientVisitDetail.episode.startStopReason_id
    );
  if (this.curPatientVisitDetail.episode.episodeType === null)
    this.curPatientVisitDetail.episode.episodeType = EpisodeType.find(
      this.curPatientVisitDetail.episode.episodeType_id
    );
  if (this.curPatientVisitDetail.episode.clinicSector === null)
    this.curPatientVisitDetail.episode.clinicSector = ClinicSector.find(
      this.curPatientVisitDetail.episode.clinicSector_id
    );
  SessionStorage.set(
    this.curPatientVisitDetail.episode.patientServiceIdentifier.service.code,
    this.curPatientVisitDetail
...
const setCurVisitDetails = () => {
  this.spetialPrescription = false;
  this.mustBeSpetial = false;
  this.showServiceDrugsManagement = false;
  this.curPatientVisitDetail = new PatientVisitDetails(
    SessionStorage.getItem(this.selectedClinicalService.code)
  );
  this.visitClone = new PatientVisitDetails(
    JSON.parse(JSON.stringify(this.curPatientVisitDetail))
  );
    ...
    if (!(this.isNewPackStep || this.isEditPackStep)) {
      ...
      if (identifier.lastVisitPrescription() !== null) {
        ...
        if (prescription.leftDuration > 0) {
          ...
          this.displayAlert(
            'confirmation',
            'O paciente possui uma prescrição válida para o serviço de ' +
              identifier.service.description +
              ', deseja anular a mesma e continuar com a criação da nova prescrição especial?'
          );
        }
        this.validateDate();
      }
...
const checkClinicalServiceAttr = (attr) => {
  if (
    this.selectedClinicalService === '' ||
    this.selectedClinicalService === null ||
    this.selectedClinicalService === undefined
  )
    return false;
...
const updatePrescribedDrugs = (
  ...
  if (
    ...
    this.curPatientVisitDetail.prescription.prescribedDrugs = JSON.parse(
      JSON.stringify(prescribedDrugs)
    );
    if (pickupDate !== null && pickupDate !== undefined)
      this.curPatientVisitDetail.pack.packDate =
        this.getJSDateFromDDMMYYY(pickupDate);
    if (pickupDate !== undefined)
      this.curPatientVisitDetail.pack.pickupDate =
        this.getJSDateFromDDMMYYY(pickupDate);
    if (nextPDate !== undefined)
      this.curPatientVisitDetail.pack.nextPickUpDate =
        this.getJSDateFromDDMMYYY(nextPDate);
    if (duration !== undefined)
      this.curPatientVisitDetail.pack.weeksSupply = duration.weeks;
  }
};
const validateForm = () => {
  ...
  if (this.hasTherapeuticalRegimen) {
    Array.isArray(this.$refs.therapeuticRegimenRef)
      ? this.$refs.therapeuticRegimenRef[0].validate()
      : this.$refs.therapeuticRegimenRef.validate();
  }
  if (this.hasTherapeuticalLine) {
    Array.isArray(this.$refs.therapeuticLine)
      ? this.$refs.therapeuticLine[0].validate()
      : this.$refs.therapeuticLine.validate();
  }
  if (this.spetialPrescription) {
    Array.isArray(this.$refs.spetialMotive)
      ? this.$refs.spetialMotive[0].validate()
      : this.$refs.spetialMotive.validate();
  }
  Array.isArray(this.$refs.duration)
    ? this.$refs.duration[0].validate()
    : this.$refs.duration.validate();
  Array.isArray(this.$refs.doctor)
    ? this.$refs.doctor[0].validate()
    : this.$refs.doctor.validate();
  Array.isArray(this.$refs.patientStatus)
    ? this.$refs.patientStatus[0].validate()
    : this.$refs.patientStatus.validate();
  // if (this.hasPatientType) this.$refs.patientType.validate()
  Array.isArray(this.$refs.dispenseType)
    ? this.$refs.dispenseType[0].validate()
    : this.$refs.dispenseType.validate();
  if (
    !this.$refs.patientStatus.hasError &&
    ...
    if (
      !this.isValidDate(
        String(this.getDateFromHyphenDDMMYYYY(this.prescriptionDate))
      )
    ) {
      this.displayAlert('error', 'A data da prescrição é inválida.');
    } else if (
      this.getYYYYMMDDFromJSDate(
        this.getDateFromHyphenDDMMYYYY(this.prescriptionDate)
      ) <
      this.getYYYYMMDDFromJSDate(this.curPatientVisitDetail.episode.episodeDate)
    ) {
      this.displayAlert(
        'error',
      ...
      ) > this.getYYYYMMDDFromJSDate(moment())
    ) {
      this.displayAlert(
        'error',
...
const doValidationToDispense = () => {
  ...
  this.clinicalServices.forEach((service) => {
    if (
      SessionStorage.getItem(service.code).prescription.prescribedDrugs.length >
      0
    ) {
  ...
  if (hasSomePrescribed) {
    Object.keys(this.curPatientVisitDetails).forEach(
      function (k) {
        ...
        const tempPvd = new PatientVisitDetails(
          JSON.parse(JSON.stringify(visitDetails))
        );
        if (this.isNewPackStep) {
          tempPvd.prescription.leftDuration = Number(
            tempPvd.prescription.leftDuration
          );
        } else {
          tempPvd.prescription.leftDuration = Number(
            Number(tempPvd.prescription.duration.weeks) / 4
          );
        }
        visitDetails.prescription.leftDuration =
          tempPvd.prescription.leftDuration;
        if (
          !visitDetails.createPackLater &&
          visitDetails.prescription.prescribedDrugs.length > 0
        ) {
          hasSomePrescribed = true;
          if (Number(visitDetails.pack.weeksSupply) <= 0) {
            hasError = true;
            const errorMsg =
              'Por favor indicar o período para o qual pretende efectuar a dispensa dos medicamento de';
            error =
              error === ''
                ? errorMsg + ' ' + this.selectedClinicalService.description
                : error +
                  ', ' +
                  errorMsg +
                  ' ' +
                  this.selectedClinicalService.description;
          } else if (
            Number(visitDetails.pack.weeksSupply / 4) >
            Number(tempPvd.prescription.leftDuration)
          ) {
            hasError = true;
            const errorMsg =
              'O Período para o qual pretende efectuar a dispensa é maior que o período remanescente na prescrição de';
            error =
              error === ''
                ? errorMsg + ' ' + this.selectedClinicalService.description
                : error +
            ...
            hasError = true;
            const errorMsg =
              'O Período para o qual pretende efectuar a dispensa é maior que o período de validade da prescrição de';
            error =
              error === ''
                ? errorMsg + ' ' + this.selectedClinicalService.description
                : error +
            ...
            new Date(visitDetails.pack.pickupDate) >
            this.getYYYYMMDDFromJSDate(moment())
          ) {
            hasError = true;
            error =
              'A data de levantamento indicada é maior que a data corrente';
          } else if (
            this.dispenseMode === null ||
            this.dispenseMode === undefined ||
            this.dispenseMode === ''
          ) {
            hasError = true;
            ...
            if (
              ...
              console.log(
                'visit date 1 ',
                visitDetails.prescription.prescriptionDate
              );
              visitDetails.patientVisit.visitDate = new Date(
                visitDetails.prescription.prescriptionDate
              );
    ...
    this.displayAlert(
      'error',
      'Por favor indicar os medicamentos a dispensar.'
    );
  }
  if (hasSomePrescribed) {
    if (!hasError) {
...
const initPackageStock = (stock, drug, quantitySupplied) => {
...
const generatePacks = (visitDetails) => {
  Object.keys(visitDetails.prescription.prescribedDrugs).forEach(function (k) {
    ...
    const stocks = Stock.query()
      .with(['clinic.*'])
      .with([
        'entrance.clinic.province',
        'entrance.clinic.district.province',
        'entrance.clinic.facilityType',
      ])
      .with([
        'center.clinic.province',
        'center.clinic.district.province',
        'center.clinic.facilityType',
      ])
    ...
    const validStock = stocks.filter((item) => {
      return (
        moment(item.expireDate).format('YYYY-MM-DD') >
          moment().format('YYYY-MM-DD') && item.stockMoviment > 0
      );
    });
    if (validStock !== undefined && validStock !== null) {
      ...
      if (avalivableStock >= qtyPrescribed) {
        ...
        return (
          'Não existe stock suficiente do medicamento ' +
          prescribedDrug.drug.name
        );
      }
    } else {
      return (
        'Não existe stock suficiente do medicamento ' + prescribedDrug.drug.name
      );
    }
  });
};
const proccedToDispense = () => {
  ...
  if (this.isNewPackStep || this.isEditPackStep || this.isFirstPack) {
    ...
    this.curPatientVisitDetails[0].prescription.prescriptionDetails[0].prescription =
      null;
  }
  this.patientVisit.patientVisitDetails = [];
  if (this.member !== null && this.member !== undefined) {
    this.curPatientVisitDetails[0].prescription.prescriptionDate =
      this.getJSDateFromDDMMYYY(this.prescriptionDate);
    const duration = Number(
      this.curPatientVisitDetails[0].prescription.duration.weeks / 4
    );
    ...
    memberPrescription.member.patient = Patient.query()
      ...
      .first();
    memberPrescription.prescription.doctor = Doctor.query()
      .with([
        'clinic.province',
        'clinic.district.province',
        'clinic.facilityType',
      ])
      .where('id', memberPrescription.prescription.doctor.id)
      .first();
    memberPrescription.prescription.prescriptionDetails[0].therapeuticRegimen =
      TherapeuticRegimen.query()
    ...
    if (this.mobile) {
      ...
      Prescription.apiSave(memberPrescription.prescription).then((resp) => {
        ...
        memberPrescription.prescription.$id = resp.response.data.id;
        memberPrescription.prescription.prescribedDrugs = [];
        memberPrescription.prescription.prescriptionDetails[0].id =
          resp.response.data.prescriptionDetails[0].id;
        ...
        GroupMemberPrescription.apiSave(memberPrescription).then((resp1) => {
          GroupMemberPrescription.apiFetchByMemberId(this.member.id).then(
            (respd) => {
              if (respd.response.status === 200) {
                Prescription.apiFetchById(
                  respd.response.data.prescription.id
                ).then((resp2) => {
      ...
      function (k) {
        ...
        if (visitDetails.prescription.prescribedDrugs.length > 0) {
          if (!visitDetails.createPackLater) {
            ...
            visitDetails.pack.syncStatus =
              this.patient.his_id.length > 10 ? 'R' : 'N';
          } else {
...
const doMobileSave = () => {
  if (this.isEditPackStep) {
    PatientVisit.localDbGetById(this.patientVisit.id).then((patientVisit) => {
      ...
      PatientVisit.localDbUpdate(JSON.parse(JSON.stringify(this.patientVisit)))
        .then((response) => {
    ...
    PatientVisit.localDbAdd(JSON.parse(JSON.stringify(this.patientVisit)))
      .then((response) => {
        this.loadVitisToVueX(JSON.parse(JSON.stringify(this.patientVisit)));
        this.addOrRemoveStockPackagedDrugs(this.patientVisit, true);
        this.displayAlert(
          'info',
          !this.hasVisitsToPackNow
            ? 'Prescrição gravada com sucesso.'
            : 'Dispensa efectuada com sucesso.'
        );
      })
      .catch((error) => {
        const listErrors = [];
...
const doWebSave = () => {
  ...
  if (!this.isEditPackStep) {
    ...
    PatientVisit.apiUpdate(JSON.parse(JSON.stringify(this.patientVisit)))
      .then((resp) => {
        const pv = JSON.parse(JSON.stringify(this.patientVisit));
...
const loadVitisToVueX = async (pv) => {
...
const fecthVisit = (id) => {
  PatientVisit.apiFetchById(id).then((resp) => {
    ...
    this.fecthPrescription(
      resp.response.data.patientVisitDetails[0].prescription.id
    );
    if (resp.response.data.patientVisitDetails[0].pack !== null)
      this.fecthPack(resp.response.data.patientVisitDetails[0].pack.id);
  });
};
const fecthVisitDetails = (id) => {
  PatientVisitDetails.apiFetchById(id).then((resp) => {
    console.log(resp)
  });
};
const fecthPrescription = (id) => {
  Prescription.apiFetchById(id).then((resp) => {
    PrescriptionDetail.apiFetchById(
      resp.response.data.prescriptionDetails[0].id
    );
  });
};
...
const addOrRemoveStockPackagedDrugs = (patientVisit, addDispense) => {
...
const filterFnspetialPrescriptionMotives = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionsspetialPrescriptionMotives = stringOptions.map(
...
const filterFntherapeuticRegimens = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionstherapeuticRegimens = stringOptions.map(
...
const filterFntherapeuticLines = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionstherapeuticLines = stringOptions.map(
...
const filterFnreasonsForUpdate = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionsreasonsForUpdate = stringOptions.map(
...
const filterFndispenseTypes = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionsdispenseTypes = stringOptions.map(
...
const filterFnpatientStatus = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      this.optionspatientStatus = stringOptions.map(
...
// Computed
const hasPrescribdDrugs = computed(() => {
  return this.checkPrescribedDrugs();
});
const showDispenseMd = computed(() => {
  return (
    !this.inFormEdition && (this.member === null || this.member === undefined)
  );
});
const dispenseModes = computed(() => {
  return DispenseMode.query()
    .where((dispenseMode) => {
...
const hasVisitsToPackLater = computed(() => {
  const pvds = [];
...
const hasVisitsToPackNow = computed(() => {
  const pvds = [];
  this.clinicalServices.forEach((cls) => {
    pvds.push(new PatientVisitDetails(SessionStorage.getItem(cls.code)));
  });
...
const curPrescription = computed(() => {
  return this.curPatientVisitDetail.prescription;
});
const curPack = computed(() => {
  return this.curPatientVisitDetail.pack;
});
const curPrescriptionDetails = computed(() => {
  if (this.curPrescription === null || this.curPrescription === undefined)
    return null;
