...
export default {
 ...
 const stocks = await StockService.localDbGetUsedStock(params)
   ...
   const packs = await patientVisitService.localDbGetPacks()
        const packagedDrug = []
        const packsDate = packs.filter(pack => pack!== undefined &&
            ...
            referredStocksIds.forEach(drugStockReferredId => {
              if (drugStockReferredId === stock.id) {
                resultDrugStocksReferred.get(drugStockReferredId).forEach(referredAdjustment => {
                  const operation = StockOperationTypeService.getStockOperatinTypeById(referredAdjustment.operation.id)
    ...
    getStockOperationTypeById (id) {
      ...
      localDbAddOrUpdate (targetCopy: any) {
        return nSQL().onConnected(() => {
            nSQL(StockUsedReport.entity).query('upsert', targetCopy).exec()
        })
        },

    async localDbGetAllByReportId (reportId: any) {
        return nSQL(StockUsedReport.entity).query('select').where(['reportId', '=', reportId]).exec().then( result => {
            if (result !== undefined) {
            return result
            }
            return null
