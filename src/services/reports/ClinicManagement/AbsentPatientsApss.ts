...
export default {
  async downloadPDF(id, fileType, params) {
    ...
    if (isOnline.value) {
      const rowsAux = await Report.printReportOther(
        'absentPatientsReport',
        id
      );
  ...
  async downloadExcel(id, fileType, params) {
    ...
    cellDistrictValue.value = params.district === null ? clinic?.district.description : params?.district.description;
    cellProvinceValue.value = params.province === null ? clinic?.province.description : params?.province.description;
  ...
  createArrayOfArrayRow(rows, service) {
    ...
    for (const row in rows) {
      if(rows[row].servedService === service){
        ...
        if(rows[row].servedService === 'TB'){
          createRow.push('SIM');
        } else {
          createRow.push('NAO');
        }

        if(rows[row].servedService === 'SMI'){
          createRow.push('SIM');
        } else {
          createRow.push('NAO');
        }
