<template>
  <div ref="filterAbsentPatientSection">
    <ListHeader
      ...
      Faltosos ao levantamento para Dispensa trimestral
    </ListHeader>
    <ListHeader
      v-else
      :addVisible="false"
      ...
      Pacientes Faltosos ao levantamento para Dispensa trimestral
    </ListHeader>
...
import absentPatientsTs from 'src/services/reports/ClinicManagement/AbsentPatientsDT.ts';
...
const name = 'AbsentPatientsDT';
const props = defineProps(['selectedService', 'menuSelected', 'id', 'params']);
const totalRecords = ref(0);
const qtyProcessed = ref(0);
const report = 'FALTOSOS_AO_LEVANTAMENTO_DT';
const progress = ref(0.0);
const filterAbsentPatientSection = ref('');
const downloadingPdf = ref(false);
const downloadingXls = ref(false);
const isReportClosed = ref(false);

const closeSection = (params) => {
  filterAbsentPatientSection.value.remove();
  if (params) {
